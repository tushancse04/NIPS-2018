...
class word2vec(word2vec_mln):
	...
	def run_word2vec(self):
		...
		for w in self.model.wv.vocab:
			...
			for x in simw:
				c = simw.count(x)
				if c >= (self.TOPN - 1) and x not in common_w:
					common_w += [x]
			if len(common_w) == 0:
				continue
	...
	def replace_orig_atoms(self):
				...
				for pred_idx in self.dom_pred_map[dom]:
					...
					for atom in w2v_meta_atoms[predname]:
						if atom[obj_idx] == ow:
							...
							word2vec_meta_orig_map[w] = cw[0]

		self.word2vec_meta_orig_map = word2vec_meta_orig_map
		self.w2v_meta_atoms = {}
		...
		for p in self.w2v_meta_atoms:
			print(p,len(self.w2v_meta_atoms[p]),len(self.db.pred_atoms[p]))

		cluster_evid_file = self.w2v__cluster_db_file
		ofile = open(cluster_evid_file,'w')
		for predname in self.w2v_meta_atoms:
			for atom in self.w2v_meta_atoms[predname]:
				atomstr = predname + '('
